...
class Imaging {
  ...
  getImagingSettings(videoSourceToken, callback) {
    ...
    const soapBody = `<timg:VideoSourceToken>${videoSourceToken}</timg:VideoSourceToken>`;
  ...
  setImagingSettings(videoSourceToken, settings, callback) {
    const promise = new Promise((resolve, reject) => {
      ...
      soapBody += '</timg:ImagingSettings>';
      this.buildRequest('SetImagingSettings', soapBody, callback).then(results => {
        resolve(results);
      }).catch(error => {
        reject(error);
      });
  ...
  move(videoSourceToken, focus, callback) {
    const promise = new Promise((resolve, reject) => {
      ...
      soapBody += '</timg:Focus>';
      this.buildRequest('Move', soapBody, callback).then(results => {
        resolve(results);
      }).catch(error => {
        reject(error);
      });
